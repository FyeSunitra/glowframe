...
interface RentalAgreementGateDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onAccepted: () => void
}

export function RentalAgreementGateDialog({
  open,
  onOpenChange,
  onAccepted,
  ...
  const { data: policies = [], isLoading, isError } = useQuery({
    ...
    queryFn: async () => unwrapApiResponse(await policyService.listRequired({ locale, context: 'rental' })),
    enabled: open,
  })
  const agreement = policies.find((item) => item.type === 'rentalAgreement')
  ...
  const effectiveDate = agreement?.effectiveAt
    ? new Intl.DateTimeFormat(locale === 'th' ? 'th-TH' : 'en-US', { dateStyle: 'medium' })
      .format(new Date(agreement.effectiveAt))
    : null
      ...
      >
        ...
        <div className="min-h-0 overflow-y-auto px-5 py-5 sm:px-7 sm:py-6">
          {isLoading ? (
            <div className="flex min-h-44 items-center justify-center gap-2 text-sm text-gf-muted">
              ...
              {t.rentalAgreementLoading}
            </div>
          ) : isError || !agreement ? (
            <p className="m-0 rounded-[8px] border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              {t.rentalAgreementUnavailable}
            </p>
          ) : (
            <article>
              <div className="mb-4 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gf-muted">
                <span>{t.rentalAgreementVersion} {agreement.version}</span>
                {effectiveDate && <span>{t.rentalAgreementEffectiveDate} {effectiveDate}</span>}
              </div>
              <h2 className="m-0 text-base font-bold text-gf-brown-900">{agreement.title}</h2>
        ...
        <div className="border-t border-gf-line px-5 py-4 sm:px-7">
          <label className="flex cursor-pointer items-start gap-3 text-sm leading-6 text-gf-brown-800">
            <input
              ...
              disabled={!agreement || isLoading || isError || acceptMutation.isPending}
              className="mt-1 size-4 shrink-0 accent-[var(--gf-pink-500)]"
            />
            <span>{t.rentalAgreementConsent}</span>
          </label>
          {acceptMutation.isError && (
            <p className="mb-0 mt-3 text-sm text-red-700">{t.rentalAgreementAcceptFailed}</p>
          )}
        </div>
        ...
        <DialogFooter className="mx-0 mb-0 rounded-none px-5 py-4 sm:px-7">
          <Button variant="outline" onClick={() => handleOpenChange(false)} disabled={acceptMutation.isPending}>
            {t.rentalAgreementClose}
          </Button>
          <Button
            onClick={() => acceptMutation.mutate()}
            disabled={!accepted || !agreement || isLoading || isError || acceptMutation.isPending}
          >
            {acceptMutation.isPending ? <LoaderCircle className="animate-spin" /> : <Check />}
            {t.rentalAgreementContinue}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
